...
/**
 * TODO: Definición de {@code PlantaRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface PlantaRepository extends JpaRepository<Planta, String> {
     ...
     */
    public List<Planta> findByStrCodigo(String id);
    ...
    @Query("SELECT m FROM Planta m WHERE m.strCodigo LIKE CONCAT('%', :strSearch, '%')")
    public List<Planta> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Planta m WHERE m.strCodigo LIKE CONCAT('%', :strSearch, '%')")
    public Page<Planta> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
