/*
 * @fileoverview    {CodigoIdentificacionVialRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code CodigoIdentificacionVialRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface CodigoIdentificacionVialRepository extends JpaRepository<CodigoIdentificacionVial, String> {
    ...
    @Query("SELECT m FROM CodigoIdentificacionVial m WHERE m.strCiv LIKE CONCAT('%', :strSearch, '%')")
    public List<CodigoIdentificacionVial> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM CodigoIdentificacionVial m WHERE m.strCiv LIKE CONCAT('%', :strSearch, '%')")
    public Page<CodigoIdentificacionVial> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
