/*
 * @fileoverview    {IndicadorRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code IndicadorRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface IndicadorRepository extends JpaRepository<Indicador, String> {
    ...
    @Query("SELECT m FROM Indicador m WHERE m.strCodigo LIKE CONCAT('%', :strSearch, '%')")
    public List<Indicador> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Indicador m WHERE m.strCodigo LIKE CONCAT('%', :strSearch, '%')")
    public Page<Indicador> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
