/*
 * @fileoverview    {AyudanteRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code AyudanteRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface AyudanteRepository extends JpaRepository<Ayudante, Integer> {
     ...
     */
    public List<Ayudante> findByIntCedula(Integer id);
    ...
    @Query("SELECT m FROM Ayudante m WHERE m.intCedula LIKE CONCAT('%', :strSearch, '%')")
    public List<Ayudante> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Ayudante m WHERE m.intCedula LIKE CONCAT('%', :strSearch, '%')")
    public Page<Ayudante> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
