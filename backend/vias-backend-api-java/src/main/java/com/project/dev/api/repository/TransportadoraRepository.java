/*
 * @fileoverview    {TransportadoraRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code TransportadoraRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface TransportadoraRepository extends JpaRepository<Transportadora, String> {
     ...
     */
    public List<Transportadora> findByStrNombre(String id);
    ...
    @Query("SELECT m FROM Transportadora m WHERE m.strNombre LIKE CONCAT('%', :strSearch, '%')")
    public List<Transportadora> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Transportadora m WHERE m.strNombre LIKE CONCAT('%', :strSearch, '%')")
    public Page<Transportadora> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
